            ...
            <v-col cols="12">
                <v-card class="elevation-8 rounded-l" style="border-top: 3px solid #e83e8c;">
                    <v-card-text>
                        ...
                        <v-row justify="center" v-show="display">
                                ...
                                <v-card class="elevation-8 rounded-xl" style="border-top: 3px solid #e83e8c;">
                                    ...
                                    <v-card-text>
                                        <v-row>
                                            ...
                                            <v-col xs="12" sm="4" md="4">
                                                <h6 class="text-center">更新</h6>
                                                <h3 class="text-center">{{updated_count}} <span style="font-size:18px">行</span></h3>
                                            </v-col>
                                            <v-col xs="12" sm="4" md="4">
                                                <h6 class="text-center">失敗</h6>
                                                <h3 class="text-center">{{failure_count}} <span style="font-size:18px">行</span></h3>
                                            </v-col>
                                        </v-row>
                                    </v-card-text>
                                </v-card>
    ...
    methods:{
        ...
        saveFile(){
            if (this.file === '') {
                ...
                axios.post(`${process.env.VUE_APP_URL}/api/v1/upload_prescription`, formData , 
                { 
                headers: { 'Authorization': `Bearer ${this.$store.getters.isTokenAdmin}`, 'Content-Type': 'multipart/form-data' } 
                    ...
                    if (res.data.error_message === 'Expired Token' || res.data.error_message === 'Wrong Token') {
                        ...
                        this.updated_count = res.data.value.updated_count
                        this.failure_count = res.data.value.failure_count
                    }
                }).catch(err => {
