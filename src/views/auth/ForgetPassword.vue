          ...
          <v-col cols="12" xs="12" sm="6" md="6" class="col-12" >
            <v-card class="elevation-12 rounded-xl p-3" v-if="display" style="border-top: 3px solid #e83e8c;">
                ...
                <v-card-title>
                  ...
                  <h5 class="pt-2 pl-5">パスワードを忘れましたか</h5>
                </v-card-title>
                ...
                <v-card-text>
                  ...
                  >
                      <form @submit.prevent="sendEmail">
                          ...
                          <v-card-actions>
                              <v-spacer></v-spacer>
                              <v-btn class="rounded-l mr-2 pl-12 pr-12 pt-5 pb-5" color="primary" type="submit" :disabled="invalid">メールアドレスに再設定リンクを送る</v-btn>
                          </v-card-actions>
                      </form>
                  </validation-observer>
                </v-card-text>
  ...
  export default {
      ...
      methods: {
          ...
          async sendEmail(){
              ...
              await axios.post(`${process.env.VUE_APP_URL}/api/v1/forget_password`, {
                  'user_id': this.email
              })
              .then(res => {
                  this.$store.dispatch('setOverlay', false)
