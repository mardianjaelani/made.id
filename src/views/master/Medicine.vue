                ...
                <v-row class="text-right"  justify='end'>
                    <v-col cols="12" xs="12" sm="12" md="2">
                        <v-btn block color="secondary" class="text-white border-12" height="50px" @click="addItem()">
                            <v-icon>mdi-plus</v-icon> {{$t('labels.AddNewMedicine')}}
                        </v-btn>
                    </v-col>
                </v-row>
            </v-col>
...
<script>
    ...
    export default {
      ...
      methods:{
        ...
        async searchItems(){
            this.loading = true
            await axios.post(`${process.env.VUE_APP_URL}/api/v1/sell_entity_medicine`, {
